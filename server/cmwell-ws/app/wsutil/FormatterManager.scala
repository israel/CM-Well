...
class FormatterManager @Inject()(C: CMWellRDFHelper) extends LazyLogging {
  ...
  lazy val jsonFormatter = new JsonFormatter(innerToSimpleFieldName(_))
  lazy val prettyJsonFormatter = new PrettyJsonFormatter(innerToSimpleFieldName(_))
  lazy val yamlFormatter = new YamlFormatter(innerToSimpleFieldName(_))
  lazy val oYamlFormatter = new YamlFormatter(innerToSimpleFieldName(_))
  lazy val csvFormatter = CSVFormatter( prettyMangledField compose innerToSimpleFieldName)
  lazy val prettyCsvFormatter = new PrettyCsvFormatter(innerToSimpleFieldName(_))
  ...
  def getFormatter(format: FormatType,
    ...
    format match {
      ...
      case JsonlType if pretty => new PrettyJsonlFormatter(C.hashToUrlAndPrefix(_), { quadUrl =>
        C.getAliasForQuadUrl(quadUrl) match {
          case opt@Some(alias) => opt
          case None => Some(quadUrl)
        }
      }, callback)
      case JsonlType => new JsonlFormatter(C.hashToUrlAndPrefix(_), Some.apply, callback)
      ...
      case RdfType(rdfFlavor) => {
        ...
        if (rdfFormatterMap.contains(key)) rdfFormatterMap(key)
          ...
          val newFormatter = rdfFlavor match {
            ...
            case TriGFlavor => new TriGFormatter(host, fieldTranslatorForRichRDF, C.getAliasForQuadUrl(_), withoutMeta, filterOutBlanks, forceUniqueness)
            case TriXFlavor => new TriXFormatter(host, fieldTranslatorForRichRDF, C.getAliasForQuadUrl(_), withoutMeta, filterOutBlanks, forceUniqueness)
            case JsonLDQFlavor => JsonLDQFormatter(host, fieldTranslatorForRichRDF, C.getAliasForQuadUrl(_), withoutMeta, filterOutBlanks, forceUniqueness, pretty, callback)
          }
